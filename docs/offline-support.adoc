...
:github-branch: update-architecture-docs
...
syncing data back to the server once back online.
...
title Requests when browser is online

participant "Front-End" as frontend
participant "Service Worker" as sw

frontend -> sw: fetch("/campaigns/1")
sw -> server: fetch("/campaigns/1")
server -> sw: HTML / JavaScript
sw -> frontend: HTML / JavaScript
