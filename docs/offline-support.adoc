...
=== Attack scenarios (Related to offline work)
...
which will be used to encrypt the data before writting it to disk. This approach
...
specifically by the offline working mechanish. This ensures that any data posted
to the server after offline working has a valid CSRF token. This attack and our
approach to protecting against it is detailed in the
