...
The service's current design is for SAIS team members to be able to select
specific campaigns to be saved (after being encrypted) for offline use. The user
...
offline, and implementing caching as appropriate.
...
ifdef::env-github[]
image::{diagram-path-url}/diagrams/saving-campaign-for-offline-sequence-diagram.puml[Saving campaign for offline sequence diagram]
endif::[]
