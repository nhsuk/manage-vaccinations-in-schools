...
describe AppActivityLogComponent do
  ...
  shared_examples "card" do |title:, date:, notes: nil, by: nil, programme: nil|
    before { travel_to Date.new(2026, 1, 1) }
  ...
  describe "decision expiration events" do
    ...
    let(:hpv_vaccine) do
      Vaccine.find_by(programme: hpv_programme) ||
        create(:vaccine, :gardasil, programme: hpv_programme)
    end
    let(:flu_vaccine) do
      Vaccine.find_by(programme: flu_programme) ||
        create(:vaccine, :fluenz, programme: flu_programme)
    end

    before do
      # Mock the current date to ensure the previous academic year is considered expired
      allow(Date).to receive(:current).and_return(
        Date.new(AcademicYear.current, 9, 1)
      )
    end

    context "with expired consent, triage, and PSD" do
      before do
        # Create a consent for the previous academic year
        create(
          :consent,
          :given,
          ...
          submitted_at: Time.zone.parse("#{previous_academic_year}-05-30 12:00")
        )

        # Create a triage for the previous academic year
        create(
          :triage,
          :ready_to_vaccinate,
          programme: hpv_programme,
          patient:,
          academic_year: previous_academic_year,
          created_at: Time.zone.parse("#{previous_academic_year}-05-30 14:30"),
          performed_by: user
        )

        # Create a patient specific direction for the previous academic year
        create(
          ...
          vaccine: hpv_vaccine,
          academic_year: previous_academic_year,
          created_at: Time.zone.parse("#{previous_academic_year}-05-30 15:00")
        )
      end
    ...
    context "with only expired PSD" do
      before do
        # Create only a patient specific direction for the previous academic year
        create(
          :patient_specific_direction,
          programme: programmes.first,
    ...
    context "with vaccinated patient" do
      before do
        ...
        # Find or create a vaccine for the HPV programme
        vaccine =
          Vaccine.find_by(programme: programmes.first) ||
            create(:vaccine, :gardasil, programme: programmes.first)

        # Create a vaccination record for the patient
        create(
          :vaccination_record,
          programme: programmes.first,
          patient:,
          ...
          vaccine: vaccine,
          performed_at:
            Time.zone.parse("#{previous_academic_year}-05-31 12:00"),
          performed_by: user
        )

        # Create a PSD for another programme that will expire
        create(
          :patient_specific_direction,
          programme: programmes.second,
