...
describe VaccinationRecord do
  ...
  describe "#create_or_update_reporting_api_vaccination_event" do
    ...
    let(:school) { create(:school, gias_local_authority_code: 999_999) }
    let(:patient) do
      create(:patient, address_postcode: "DH1 3LH", school: school)
    end
    let(:local_authority) do
      create(:local_authority, gss_code: "D000000001", gias_code: 999_999)
    end
    ...
    context "when a reporting_api_vaccination_event already exists with this vaccination_record as source" do
      ...
      before { vaccination_record.update!(outcome: :administered) }
      ...
      it "does not create a new reporting_api_vaccination_event" do
        ...
        }.not_to change(ReportingAPI::VaccinationEvent, :count)
      end

      it "updates the existing reporting_api_vaccination_event" do
        vaccination_record.create_or_update_reporting_api_vaccination_event!

        expect(existing_event.reload.vaccination_record_outcome).to eq(
          "administered"
        )
      end
    end
    ...
    context "when no reporting_api_vaccination_event exists with this vaccination_record as source" do
      it "creates a new reporting_api_vaccination_event" do
        expect {
          vaccination_record.create_or_update_reporting_api_vaccination_event!
        }.to change(ReportingAPI::VaccinationEvent, :count).by(1)
      end
      ...
      describe "the created VaccinationEvent" do
        ...
        let(:event) do
          ReportingAPI::VaccinationEvent.find_by(
            source_id: vaccination_record.id
          )
        end
        ...
        describe "denormalized relation properties" do
          ...
          it "has patient_school_ attributes set" do
            expect(event).to have_attributes(
              ...
              patient_school_gias_local_authority_code:
          ...
          it "has patient_school_local_authority_ attributes set" do
            expect(event).to have_attributes(
              patient_school_local_authority_short_name:
                local_authority.short_name,
              patient_school_local_authority_mhclg_code:
                local_authority.mhclg_code
            )
          end

          it "has patient_local_authority_from_postcode_ attributes set" do
            expect(event).to have_attributes(
              patient_local_authority_from_postcode_short_name:
                patient.local_authority_from_postcode.short_name,
              patient_local_authority_from_postcode_mhclg_code:
                patient.local_authority_from_postcode.mhclg_code
            )
          end
