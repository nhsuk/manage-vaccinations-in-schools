...
# == Schema Information
#
# Table name: patient_session_vaccination_statuses
#
...
# Indexes
#
#  idx_on_patient_session_id_programme_id_520c97058b     (patient_session_id,programme_id) UNIQUE
#  index_patient_session_vaccination_statuses_on_status  (status)
#
# Foreign Keys
#
...
#
describe PatientSession::VaccinationStatus do
  subject(:patient_session_vaccination_status) do
    build(:patient_session_vaccination_status, patient_session:, programme:)
  end
  ...
  it do
    expect(patient_session_vaccination_status).to define_enum_for(
      :status
    ).with_values(
      %i[
        none_yet
  ...
  describe "#status" do
    subject(:status) { patient_session_vaccination_status.assign_status }
