...
class DummyClass
  ...
  attr_accessor :patient_nhs_number,
...
describe ReportingAPI::DenormalizingConcern do
    ...
    context "given some attributes" do
      ...
      context "which are ApplicationRecord instances" do
        let(:patient) do
          build(
            :patient,
            date_of_birth: "2018-02-03".to_date,
            date_of_death: "2022-03-04".to_date
          )
        end
        let(:school) { build(:school) }

        let(:attrs) { { patient: patient, school: school } }
      ...
      context "which do not exist in the ReportableEvent attributes" do
        let(:attrs) { { unknown_attribute_name: "some value" } }
