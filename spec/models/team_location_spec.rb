...
describe TeamLocation do
  subject(:team_location) { build(:team_location) }

  describe "associations" do
    it { should belong_to(:team) }
    it { should belong_to(:location) }
    it { should belong_to(:subteam).optional }
  end

  describe "validations" do
    subject(:team_location) { build(:team_location, team:, subteam:) }
    ...
    context "when the subteam belongs to the team" do
      let(:subteam) { create(:subteam, team:) }

      it { should be_valid }
    end

    context "when subteam doesn't belong to the team" do
