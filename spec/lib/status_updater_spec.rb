...
describe StatusUpdater do
  ...
  context "with an HPV session and ineligible patient" do
    ...
    it "doesn't create any patient session vaccination statuses" do
      expect { call }.not_to change(PatientSession::VaccinationStatus, :count)
    end
  end
  ...
  context "with an HPV session and eligible patient" do
    ...
    it "creates a patient session vaccination status" do
      expect { call }.to change(
        patient_session.vaccination_statuses,
        :count
      ).by(1)
      expect(patient_session.vaccination_statuses.first).to be_none_yet
    end
  end
  ...
  context "with an doubles session and eligible patient" do
    ...
    it "creates a patient session vaccination status for both programmes" do
      expect { call }.to change(
        patient_session.vaccination_statuses,
        :count
      ).by(2)
      expect(patient_session.vaccination_statuses.first).to be_none_yet
      expect(patient_session.vaccination_statuses.second).to be_none_yet
    end
  end
end
