...
describe PatientImporter::DataProcessor do
  ...
  describe "#call" do
    ...
    describe "parents and relationships result" do
      let(:result) { processed_patient_data.process }

      describe "with a single parent in the row data" do
        it "returns a new parent with correct attributes" do
          expect(processed_patient_data.parents.first).to have_attributes(
            email: "mary.smith@example.com",
            phone: "07412 345678"
          )
        end

        it "returns a single unknown parent relationship" do
          expect(processed_patient_data.parent_relationships.count).to eq(1)
          expect(
            processed_patient_data.parent_relationships.first
          ).to be_unknown
        end

        context "when the parent already exists in the system" do
          ...
          it "uses the existing parent record instead of creating a new one" do
            expect(processed_patient_data.parents.first).to eq(existing_parent)
          end
        end
      end

      describe "with two parents" do
        let(:row_data) do
          valid_row_data.merge(
            parent_2_email: "peter.smith@example.com",
            parent_2_phone: "07412345678"
          )
        end

        it "returns two unknown parent relationships" do
          expect(processed_patient_data.parent_relationships.count).to eq(2)
          expect(
            processed_patient_data.parent_relationships.first
          ).to be_unknown
          expect(
            processed_patient_data.parent_relationships.second
          ).to be_unknown
        end
      end
    end
