...
describe FHIRMapper::VaccinationRecord do
  ...
  describe "#from_fhir_record" do
    ...
    shared_examples "a mapped vaccination record (common fields)" do
      ...
      # TODO: This isn't included in the actual fhir record; it's in the wider response from the search
      # its(:nhs_immunisations_api_id) { pending("implementation") || fail }
      # TODO: Check that the etag is definitely not included in the search response
      # its(:nhs_immunisations_etag) { pending("implementation") || fail }
      # TODO: add source to vaccination_record
      its(:source) { pending("implementation") || raise }
