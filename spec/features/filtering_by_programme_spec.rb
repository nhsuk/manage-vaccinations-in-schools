...
describe "Filtering" do
  ...
  def and_i_see_all_the_statuses
    expect(page).to have_content("HPVNo outcome").exactly(2).times
    expect(page).to have_content("MenACWYNo outcome").once
  end
  ...
  def and_i_see_only_hpv_statuses_for_all_patients
    expect(page).to have_content("HPVNo outcome").exactly(2).times
    expect(page).not_to have_content("MenACWYNo outcome")
  end
  ...
  def and_i_see_only_the_menacwy_statuses
    expect(page).not_to have_content("HPVNo outcome")
    expect(page).to have_content("MenACWYNo outcome").once
  end
