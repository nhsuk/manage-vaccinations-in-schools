...
describe "Download school moves" do
  ...
  def when_i_click_on_download_records
    click_on I18n.t("school_moves.index.download")
  end

  def and_i_click_continue
    click_on I18n.t("school_moves.download.dates.submit")
  end

  def when_i_click_download_csv
    click_on I18n.t("school_moves.download.confirm.submit")
  end

  def then_i_get_a_csv_file_with_expected_row_count(expected_count)
    expect(page).to have_content(CSVSchoolMoves::HEADERS.join(","))
  ...
  def then_i_should_see_a_confirmation_page
    check_summary_list_row(
      I18n.t("school_moves.download.confirm.date_from"),
      I18n.t("school_moves.download.confirm.date_from_not_specified")
    )
    check_summary_list_row(
      I18n.t("school_moves.download.confirm.date_to"),
      I18n.t("school_moves.download.confirm.date_to_not_specified")
    )
    check_summary_list_row(I18n.t("school_moves.download.confirm.records"), "2")
  end
  ...
  def then_i_should_see_a_confirmation_page_with_dates
    ...
    check_summary_list_row(I18n.t("school_moves.download.confirm.records"), "1")
  end
