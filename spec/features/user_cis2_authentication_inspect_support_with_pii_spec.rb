...
describe "Inspect tools with PII access", :cis2 do
  scenario "Support user with PII access can view timeline with functional PII checkbox" do
    ...
    then_i_see_the_timeline_with_functional_pii_checkbox
  end
  ...
  scenario "Support user with PII access can view graph with functional PII checkbox" do
    ...
    then_i_see_the_graph_with_functional_pii_checkbox
  end
  ...
  def then_i_see_the_timeline_with_functional_pii_checkbox
    expect(page).to have_content("Customise timeline")

    # Verify PII checkbox is not disabled for users with PII access
    expect(page).to have_field("Show PII", disabled: false)
  end

  def then_i_see_the_graph_with_functional_pii_checkbox
