...
describe "Edit session programmes" do
  ...
  def then_i_see_the_warning_panel_about_unvaccinated_patients
    ...
    expect(page).to have_content(
      "0% of children in Years 9, 10, and 11 in this session have " \
        "vaccination records."
    )
    expect(page).to have_content(
      "Scheduling this session now will send consent requests to 2 parents " \
        "of children in Years 9, 10, and 11 on 18 January 2024. Many of " \
        "them may be parents of already vaccinated children."
    )
  end
