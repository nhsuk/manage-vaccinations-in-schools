...
describe "mavis gias import" do
  ...
  def then_schools_are_imported_correctly
    ...
    expect(Location.find_by(urn: "100000").status).to eq "closed"
    expect(Location.find_by(urn: "100001").status).to eq "closed"
    expect(Location.find_by(urn: "100002").status).to eq "closing"
    expect(Location.find_by(urn: "100003").status).to eq "open"
  end
end
