...
describe "Inspect timeline PII access logging", :cis2 do
  scenario "Support user with PII access creates PII access logs when visiting timeline" do
  ...
  scenario "Support user with PII access creates PII access logs for both patients when visiting timeline with comparison patient" do
    prepare_support_organisation_with_pii_access
  ...
  def prepare_hpv_programme_with_two_patients
    ...
    create(:patient_session, patient: @compare_patient, session: @session)
  end
  ...
  def then_an_access_log_entry_is_created_for_the_patient
    # Two calls are made on first page load, so in this case (since we are visiting with show_pii: true), two logs are created
