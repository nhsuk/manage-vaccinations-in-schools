...
describe "Immunisation imports" do
  ...
  scenario "User uploads an HPV file, views cohort and vaccination records" do
  ...
  scenario "User uploads a flu file, views cohort and vaccination records" do
    given_mavis_logins_are_configured
    given_i_am_signed_in_as_a_bulk_upload_user
  ...
  def given_mavis_logins_are_configured
    ...
    create(:school, team: @team, urn: 120_026)
  end
  ...
  def and_i_select_hpv
    choose "HPV"
    click_on "Continue"
  end

  def and_i_select_flu
    choose "Flu"
    click_on "Continue"
  end
  ...
  def and_i_upload_a_valid_hpv_file
    attach_file(
      "immunisation_import[csv]",
      "spec/fixtures/immunisation_import_bulk/valid_hpv.csv"
    )
    click_on "Continue"
    wait_for_import_to_complete(ImmunisationImport)
  end

  def and_i_upload_a_valid_flu_file
    attach_file(
      "immunisation_import[csv]",
      "spec/fixtures/immunisation_import_bulk/valid_flu.csv"
    )
    click_on "Continue"
    wait_for_import_to_complete(ImmunisationImport)
  end
  ...
  def and_i_should_see_the_vaccination_records
    ...
    expect(page).to have_content("Vaccination date 9 May 2025")
  end
