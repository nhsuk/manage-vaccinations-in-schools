...
describe CIS2LogoutConcern do
  ...
  describe "cis2_jwks_fetcher" do
    subject { klass_instance.send(:cis2_jwks_fetcher).call({}) }

    let(:klass_instance) { klass.new(params) }
    let(:jwks_uri) { "http://localhost:4000/jwks" }
    let(:jwks_response) { { keys: [{ kid: "key1", use: "sig" }] }.to_json }

    before do
      allow(klass_instance).to receive(:jwks_uri).and_return(jwks_uri)
  ...
  describe "cis2_openid_configuration" do
    subject { klass.new(params).send(:cis2_openid_configuration) }
