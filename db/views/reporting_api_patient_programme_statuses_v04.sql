WITH base_data AS (
  SELECT
    ...
    CONCAT(p.id, '-', s_programme_type::TEXT, '-', t.id, '-', s.academic_year) AS id,
  ...
  ROW_NUMBER() OVER (
    PARTITION BY p.id, s_programme_type, t.id, s.academic_year
...
CROSS JOIN unnest(s.programme_types) as s_programme_type
...
LEFT JOIN (
  SELECT DISTINCT vr.patient_id, vr.programme_type, vr_s.academic_year
...
) vr_any ON vr_any.patient_id = p.id AND vr_any.programme_type = s_programme_type AND vr_any.academic_year = s.academic_year
...
LEFT JOIN (
  ...
  AND vr_sais_current.programme_type = s_programme_type
...
LEFT JOIN (
  SELECT DISTINCT vr.patient_id, vr.programme_type, COALESCE(vr_s.academic_year, EXTRACT(YEAR FROM vr.performed_at)) AS academic_year
  FROM vaccination_records vr
...
LEFT JOIN (
  SELECT DISTINCT vr.patient_id, vr.programme_type, EXTRACT(YEAR FROM vr.performed_at) AS academic_year
  FROM vaccination_records vr
  WHERE vr.discarded_at IS NULL
...
LEFT JOIN (
  ...
  WHERE vr.discarded_at IS NULL
    AND vr.outcome = 0 -- administered
  GROUP BY vr.patient_id, vr.programme_type
) vr_counts ON vr_counts.patient_id = p.id AND vr_counts.programme_type = s_programme_type
...
LEFT JOIN (
  ...
  WHERE vr.discarded_at IS NULL
    AND vr.outcome = 0 -- administered
  GROUP BY vr.patient_id, vr.programme_type, vr_s.team_id
) vr_recent ON vr_recent.patient_id = p.id AND vr_recent.programme_type = s_programme_type AND vr_recent.team_id = t.id
...
LEFT JOIN LATERAL (
  ...
  WHERE pcs.patient_id = p.id
    AND pcs.programme_type = s_programme_type
