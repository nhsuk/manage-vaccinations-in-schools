...
if [[ $(grep -ce "aws_ecs_task_definition\.task_definition.*replaced" "$tf_stdout") -eq 1 ]]; then
  echo "Task definition is being replaced"
else
  echo "Task definition is not being replaced, aborting."
  exit 1
fi
if [[ $(grep -ce "aws_s3_object\.appspec_object.*updated in-place" "$tf_stdout") -eq 1 ]]; then
  echo "S3 bucket object is being replaced"
else
  echo "S3 bucket object is not being replaced, aborting."
  exit 1
fi
if [[ $(grep -c "1 to add, 1 to change, 1 to destroy." "$tf_stdout") -eq 1 ]]; then
