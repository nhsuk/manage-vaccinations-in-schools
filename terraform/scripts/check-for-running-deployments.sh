...
APPLICATION_NAME=$(terraform output -json codedeploy_application_name 2>/dev/null | tr -d '"')
echo "Application Name: $APPLICATION_NAME"
APPLICATION_GROUP=$(terraform output -json codedeploy_deployment_group_name 2>/dev/null | tr -d '"')
