...
class Patient < ApplicationRecord
  ...
  def self.match_existing(
    ...
    nhs_number = normalise_match_existing_nhs_number(nhs_number)
    address_postcode = normalise_match_existing_postcode(address_postcode)

    unless candidates.nil?
      return(
        match_existing_from_candidates(
          candidates,
          nhs_number:,
          given_name:,
          family_name:,
  ...
  def self.match_existing_from_database(
    nhs_number:,
    given_name:,
    family_name:,
    date_of_birth:,
  ...
  def self.match_existing_from_candidates(
    candidates,
    nhs_number:,
    given_name:,
    family_name:,
    ...
    if nhs_number.present?
      matched =
        candidates.find do
          normalise_match_existing_nhs_number(it.nhs_number) == nhs_number
        end
    ...
    results =
      candidates.select do
        patient_nhs_number = normalise_match_existing_nhs_number(it.nhs_number)
        ...
        if address_postcode.present?
          postcode_matches =
            normalise_match_existing_postcode(it.address_postcode) ==
              address_postcode
  ...
  def self.narrow_to_single_exact_match_if_possible(
    ...
    exact_results =
      results.select do
        normalise_match_existing_postcode(it.address_postcode) ==
          address_postcode && it.given_name.to_s.casecmp?(given_name) &&
  ...
  def self.normalise_match_existing_nhs_number(nhs_number)
    nhs_number&.normalise_whitespace&.gsub(/\s/, "")
  end

  def self.normalise_match_existing_postcode(address_postcode)
  ...
  private_class_method :match_existing_from_database,
                       :match_existing_from_candidates,
                       :narrow_to_single_exact_match_if_possible,
                       :normalise_match_existing_nhs_number,
                       :normalise_match_existing_postcode
