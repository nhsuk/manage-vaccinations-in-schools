...
class ImmunisationImportRow
  ...
  def batch
    return unless valid?
    return unless administered && vaccine
    return if batch_expiry.blank? && batch_name.blank?
  ...
  def validate_batch_expiry
    if administered
      ...
      elsif offline_recording? || batch_name.present?
  ...
  def validate_batch_name
    if administered
      if (offline_recording? || batch_expiry.present?) && batch_name.blank?
        errors.add(:batch_name, :blank)
      end
