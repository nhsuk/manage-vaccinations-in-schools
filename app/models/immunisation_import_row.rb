...
class ImmunisationImportRow
  ...
  def historical? = type == :historical
  ...
  def vaccine
    ...
    @vaccine ||=
      team
        .vaccines
        .includes(:programme)
        .find_by(upload_name: vaccine_upload_name) ||
        team
          .vaccines
          .includes(:programme)
          .find_by(nivs_name: vaccine_upload_name)
  end
  ...
  def validate_clinic_name
    clinic_name_required = offline_recording? && is_community_setting? && historical?
  ...
  def validate_dose_sequence
    ...
    elsif administered && ((offline_recording? && default_dose_sequence.present? ) || bulk_hpv?)
      if field.nil?
        errors.add(
          :base,
