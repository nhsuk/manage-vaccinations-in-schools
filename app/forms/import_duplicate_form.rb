...
class ImportDuplicateForm
  ...
  def apply_pending_changes!
    if object.respond_to?(:patient)
      object.patient.apply_pending_changes! do |pending_changes, record|
        update_patient_family_and_school_associations(pending_changes, record)
      end
    end
  ...
  def keep_both_changes!
    if can_keep_both?
      object.apply_pending_changes_to_new_record! do |pending_changes, new_record|
        update_patient_family_and_school_associations(
          pending_changes,
          new_record
        )
      end
  ...
  def update_patient_family_and_school_associations(pending_changes, new_record)
    family_connections =
