...
class SchoolRemovalForBlmkAndNw
  ...
  def warn_about_incorrect_state
    warnings = []
    warnings +=
      warnings_for_schools_with_no_sessions(
        subteams: [bedfordshire_luton_and_milton_keynes, norfolk_and_waveney]
      )

    # These warnings for wrong schools are redundant when the validation is run after the script is performed,
    # but are useful to verify the current state is indeed in an incorrect state before running the script.
    warnings +=
      warnings_for_wrong_schools_assigned(
        subteam: bedfordshire_luton_and_milton_keynes,
        desired_schools: correct_blkm_schools
    ...
    subteams.each do |subteam|
      subteam.schools.each do |school|
        if school.sessions.empty?
          warnings << "No sessions for #{school.name} (URN #{school.urn})"
        end
      end
    end
    warnings
