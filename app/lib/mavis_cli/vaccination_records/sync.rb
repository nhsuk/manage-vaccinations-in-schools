  ...
  module VaccinationRecords
    class Sync < Dry::CLI::Command
      ...
      def call(vaccination_record_id:, **)
        ...
        if vaccination_record.synced_at.present?
          puts "Vaccination record #{vaccination_record_id} has already been synced at #{vaccination_record.synced_at}"
          return
        end

        SyncVaccinationRecordJob.perform_now(vaccination_record)
