...
class AppSearchComponent < ViewComponent::Base
  erb_template <<-ERB
    <%= render AppCardComponent.new(filters: true) do |card| %>
      ...
      <%= form_with model: form, url:, method: :get, builder: GOVUKDesignSystemFormBuilder::FormBuilder do |f| %>
        ...
        <% if record_status %>
          <%= f.govuk_radio_buttons_fieldset :record_status, legend: { text: "Vaccination status", size: "s" } do %>
            <%= f.govuk_radio_button :record_status, "", label: { text: "Any" } %>
            <% PatientSession::Record::STATUSES.each do |status| %>
              <%= f.govuk_radio_button :record_status, status, label: { text: t(status, scope: %i[patient_session status record label]) } %>
            <% end %>
          <% end %>
        <% end %>
        ...
        <% if outcome_status %>
          <%= f.govuk_radio_buttons_fieldset :outcome_status, legend: { text: "Programme outcome", size: "s" } do %>
            ...
            <% PatientSession::Outcome::STATUSES.each do |status| %>
  ...
  def show_buttons_in_details?
    !(
      consent_status || outcome_status || record_status || register_status ||
        triage_status || year_groups.any?
    )
  end
end
