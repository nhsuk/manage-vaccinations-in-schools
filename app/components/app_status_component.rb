...
class AppStatusComponent < ViewComponent::Base
  erb_template <<-ERB
    <p class="app-status app-status--<%= @colour %> <% if @small %>app-status--small<% end %>">
      <svg class="nhsuk-icon nhsuk-icon--warning" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" focusable="false" aria-hidden="true">
