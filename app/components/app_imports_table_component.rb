...
class AppImportsTableComponent < ViewComponent::Base
  ...
  def imports
    @imports ||=
      if @uploaded_files
        (
          class_import_records.uploaded_files +
            cohort_import_records.uploaded_files +
            immunisation_import_records.uploaded_files
        ).sort_by(&:created_at).reverse
      else
        (
          class_import_records.imported_records +
            cohort_import_records.imported_records +
            immunisation_import_records.imported_records
        ).sort_by(&:created_at).reverse
      end
  end
